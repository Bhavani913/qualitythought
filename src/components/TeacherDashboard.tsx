...
export default function TeacherDashboard({ onLogout }: TeacherDashboardProps) {
  ...
  // Interviews state — shared so student profile panel can show recordings
  ...
  const [newStudentCountryCode, setNewStudentCountryCode] = useState("+91");
  ...
  const handleToggleVideoAccess = async (interviewId: string, videoAccessGranted: boolean) => {
    try {
      const res = await fetch(`/api/interviews/${interviewId}/toggle-video-access`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ videoAccessGranted })
      });
      if (res.ok) {
        setInterviews(prev =>
          prev.map(iv => iv.id === interviewId ? { ...iv, videoAccessGranted } : iv)
        );
      }
    } catch (err) {
      console.error("Failed to toggle video access:", err);
    }
  };
  ...
  const fetchDB = async () => {
    ...
    try {
      ...
      // Fetch all interviews so student profile panel can show recordings
      try {
        ...
        console.error("Failed to fetch interviews:", ivErr);
      }
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (newStudentPhone && !/^\d{10}$/.test(newStudentPhone.replace(/\s/g, ""))) {
      alert("Mobile number must be exactly 10 digits (no spaces or dashes — select country code separately).");
      return;
    }

    const fullPhone = newStudentPhone ? `${newStudentCountryCode} ${newStudentPhone}` : "";
                  ...
                  <form onSubmit={handleAddStudent} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <select
                          value={newStudentCountryCode}
                          onChange={(e) => setNewStudentCountryCode(e.target.value)}
                          className="bg-slate-50 border border-slate-200 rounded px-2 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 w-28 shrink-0"
                        >
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setNewStudentPhone(digits);
                          }}
                          placeholder="10-digit number"
                          maxLength={10}
                          className="flex-1 bg-slate-50 border border-slate-200 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        />
                      </div>
                    </div>
      ...
      {selectedStudentForReview && (
        <div className="fixed inset-0 bg-slate-900/75 z-[100] flex items-center justify-center p-4 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-5xl w-full p-6 animate-zoom-in max-h-[92vh] overflow-y-auto flex flex-col gap-6">
            ...
            {(() => {
              const studentRecordings = interviews.filter(
                (iv) => iv.studentId === selectedStudentForReview.id && iv.videoUrl
              );
              if (studentRecordings.length === 0) return null;
              return (
                <div className="bg-slate-900 rounded-2xl p-5 border border-slate-800 space-y-4">
                  <div className="border-b border-slate-700 pb-3">
                    <span className="bg-indigo-600 text-white font-mono text-[9px] font-extrabold py-0.5 px-2 rounded uppercase tracking-wider inline-block">
                      Session Recordings
                    </span>
                    ...
                    <p className="text-slate-400 text-[10px] font-sans mt-0.5">
                      Webcam-recorded mock interview sessions for this student.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {studentRecordings.map((iv) => (
                      <div key={iv.id} className="bg-slate-800 rounded-xl p-3 border border-slate-700 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-mono font-bold text-indigo-300 uppercase">
                            {iv.roundType || "Technical"} · {iv.subject}
                          </span>
                        ...
                        <video
                          src={iv.videoUrl}
                          controls
                          className="w-full rounded-lg border border-slate-700 bg-black max-h-[180px]"
                        />
                        <div className="flex items-center justify-between pt-1">
                          <span className={`text-[9px] font-bold font-mono px-2 py-0.5 rounded-full ${iv.videoAccessGranted ? "bg-emerald-900 text-emerald-300" : "bg-slate-700 text-slate-400"}`}>
                            {iv.videoAccessGranted ? "✓ Student Access On" : "Student Access Off"}
                          </span>
                          <button
                            type="button"
                            onClick={() => handleToggleVideoAccess(iv.id, !iv.videoAccessGranted)}
                            className="text-[9px] font-bold font-mono bg-indigo-700 hover:bg-indigo-600 text-white px-2 py-0.5 rounded cursor-pointer transition"
                          >
                            {iv.videoAccessGranted ? "Revoke Access" : "Grant Access"}
                          </button>
                        </div>
                      </div>
                    ))}
