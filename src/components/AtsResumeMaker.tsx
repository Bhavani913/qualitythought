...
export default function AtsResumeMaker({ student }: AtsResumeMakerProps) {
  ...
  const handleEducationChange = (id: string, field: keyof EducationItem, value: string) => {
    if (field === "gpa") {
      // If user types a plain number, validate GPA ≤ 10 and percentage ≤ 100
      const numMatch = value.match(/(\d+(\.\d+)?)/);
      if (numMatch) {
        const num = parseFloat(numMatch[1]);
        if (value.toLowerCase().includes("%") || num > 10) {
          // Treat as percentage
          if (num > 100) return; // Block values over 100%
        } else {
          // Treat as GPA
          if (num > 10) return; // Block GPA over 10
        }
      }
    }
    if (field === "endDate") {
      // Block graduation years more than 2 years in the future
